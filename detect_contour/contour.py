...
def convert_rgb_to_gray(rgb_image,show):
    ...
    if show: 
        ...
        #cv2.moveWindow("Gray Image",width, 0)
    return gray_image
...
def convert_gray_to_binary(gray_image, adaptive, show):
    if adaptive: 
        ...
        _, binary_image = cv2.threshold(gray_image,85,255,cv2.THRESH_BINARY_INV)
    if show:
        cv2.imshow("Binary Image", binary_image)
#        cv2.moveWindow("Binary Image",2*width, 0)
    return binary_image    
...
#    cv2.moveWindow(image_name ,3*width, 0)
